...
int main()
{
	...
	for (int y = 0; y < 10; y++)
	{
		for (int x = 0; x < 10; x++)
		{
			tils.vertices.push_back(glm::vec2(x*1.1*tileSize, 			y*1.1*tileSize));
			tils.vertices.push_back(glm::vec2(x*1.1*tileSize, 			y*1.1*tileSize+tileSize));
			tils.vertices.push_back(glm::vec2(x*1.1*tileSize+tileSize, 	y*1.1*tileSize+tileSize));
			tils.vertices.push_back(glm::vec2(x*1.1*tileSize+tileSize, 	y*1.1*tileSize));

			tils.texCoords.push_back(glm::vec2(0.0, 0.0));
			tils.texCoords.push_back(glm::vec2(0.0, 1.0));
			tils.texCoords.push_back(glm::vec2(1.0, 1.0));
			tils.texCoords.push_back(glm::vec2(1.0, 0.0));
		}
	}
	tils.upload();
	...
	while (glfwGetWindowParam(GLFW_OPENED) && !glfwGetKey(GLFW_KEY_ESC) && !((glfwGetKey(GLFW_KEY_LCTRL) || glfwGetKey(GLFW_KEY_RCTRL)) && (glfwGetKey('C') || glfwGetKey('W') || glfwGetKey('D'))))
	{
		...
		{
			{
				glUniformMatrix4fv(drawman.mvpUniform, 1, GL_FALSE, glm::value_ptr(drawman.projection));

				glClearColor(0.1f, 0.1f, 0.1f, 1.f);
				glClear(GL_COLOR_BUFFER_BIT);

				tils.draw(drawman.posAttrib, drawman.texAttrib, GL_QUADS);
